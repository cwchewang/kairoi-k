...
def wallseg(name,x1,y1,x2,y2,z0=0,z1=HEIGHT,material=None,collection=None):
 dx=x2-x1;dy=y2-y1;L=(dx*dx+dy*dy)**.5+.05;ang=math.atan2(dy,dx);o=box(name,((x1+x2)/2,(y1+y2)/2,(z0+z1)/2),(L,T,z1-z0),material,.012,collection);o.rotation_euler[2]=ang;return o
...
WALL=polyhaven_texture('white_plaster_02','Wall Plaster',CACHE,2.6);WOOD=polyhaven_texture('oak_wood_planks','Oak Floor',CACHE,1.0);TILE=polyhaven_texture('interior_tiles','Living Tile',CACHE,1.1);BATH=polyhaven_texture('marble_tiles','Bathroom Tile',CACHE,1.4)
FRAME=mat('Window Frame',(.09,.10,.11),.28,.65);TRIM=mat('Warm White Trim',(.78,.76,.70),.42);COUNTER=mat('Countertop',(.25,.24,.22),.22);CAB=mat('Cabinet Oak',(.34,.19,.09),.38);BAL=mat('Balcony Stone',(.34,.36,.36),.75)
GLASS=bpy.data.materials.new('Architectural Glass');GLASS.use_nodes=True;b=GLASS.node_tree.nodes.get('Principled BSDF');b.inputs['Base Color'].default_value=(.72,.86,.95,1);b.inputs['Roughness'].default_value=.08;(b.inputs.get('Transmission Weight') or b.inputs.get('Transmission')).default_value=.88;b.inputs['IOR'].default_value=1.45

floor('LivingFloor',[(0,0),(3.12,0),(3.12,2.83),(4.113,2.83),(4.113,4.903),(0,4.903)],TILE);floor('BedroomFloor',[(3.12,0),(6.265,0),(6.265,2.83),(3.12,2.83)],WOOD);floor('KitchenFloor',[(4.113,2.83),(6.265,2.83),(6.265,4.903),(4.113,4.903)],BATH);floor('BathroomFloor',[(1.143,4.903),(3.18,4.903),(3.18,6.523),(1.143,6.523)],BATH);floor('BalconyFloor',[(6.265,0),(7.327,0),(7.327,4.903),(6.265,4.903)],BAL)
...
world=bpy.context.scene.world or bpy.data.worlds.new('World');bpy.context.scene.world=world;world.use_nodes=True;world.node_tree.nodes['Background'].inputs['Color'].default_value=(.11,.13,.16,1);world.node_tree.nodes['Background'].inputs['Strength'].default_value=.22
for name,loc,energy,size,color in [('LivingArea',(1.7,2.1,2.65),430,2.1,(1,.83,.65)),('BedroomArea',(4.8,1.3,2.65),260,1.6,(1,.82,.66)),('KitchenArea',(5.2,3.85,2.55),300,1.3,(1,.90,.77))]:
...
sd=bpy.data.lights.new('Sun','SUN');sd.energy=2;sd.angle=math.radians(5);sun=bpy.data.objects.new('Sun',sd);bpy.context.collection.objects.link(sun);sun.rotation_euler=(math.radians(35),math.radians(-20),math.radians(-35))
camd=bpy.data.cameras.new('Camera');cam=bpy.data.objects.new('Camera',camd);bpy.context.collection.objects.link(cam);bpy.context.scene.camera=cam;cam.location=(9,-10,8);cam.rotation_euler=(Vector((3.6,2.5,1))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.lens=45
...
scene.render.filepath=str(DIST/'preview.png');scene.render.image_settings.file_format='PNG'
try:bpy.ops.render.render(write_still=True)
except Exception as e:print('preview failed',repr(e))
print('DONE',DIST)
